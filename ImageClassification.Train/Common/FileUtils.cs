...
namespace ImageClassification
{
    public class FileUtils
    {
        ...
        {
            var imagesPath = Directory.GetFiles(folder, "*", searchOption: SearchOption.AllDirectories).Where(x => Path.GetExtension(x) == ".jpg" || Path.GetExtension(x) == ".png");

            return useFolderNameasLabel ? imagesPath.Select(imagePath => (imagePath, Directory.GetParent(imagePath).Name)):
        ...
        public static InMemoryImageData LoadInMemorySingleImageFromDirectory(string FullFilePath, bool useFolderNameAsLabel = true)
        {
            var a = new InMemoryImageData(image: File.ReadAllBytes(FullFilePath),
                    label: "none",//Directory.GetParent(FullFilePath).Name,
                    imageFileName: Path.GetFileName(FullFilePath));
            return a;
        }
